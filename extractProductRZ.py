...
# 
# Modified by Rob Zinke to accommodate x-, y- resolution specs: 
#   createParser -- added xRes and yRes options (lns 51-52)
#   prepDEM -- added to inputs in definition (ln 115)
#           -- download DEM -- added as warp options (lns 144-147)
#           -- Load DEM -- added as warp options (lns 162-165)
#   prepMask -- added inputs in definition (ln 187)
#           -- save cropped mask -- added as warp options (lns 227-230)
#           -- save cropped mask -- added as warp options (lns 276-279)
#           -- pass cropped mask -- added as warp options (lns 285-288)
#   export product -- added to inputs in definition (ln 366)
#           -- finalize metadata inputs (ln 406)
#           -- building cropped vrt -- added as warp options (lns 414-417)
#           -- mask specified -- added as warp options (lns 425-428)
#   finalize metadata -- added inputs in defintion (ln 460)
#           -- crop track extents -- added as warp options (lns 509-512)
#   Consider implementing for tropo correction
#   Main -- variables from input parser (ln 762)
#           -- definition inputs to prepMask (ln 764)
#           -- definition inputs to prepDEM (ln 770)
#           -- definition inputs to exportProducts (ln 775)
#
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
import os
import sys
...
#    parser.add_argument('-sm', '--stitchMethod', dest='stitchMethodType',  type=str, default='overlap', help="Method applied to stitch the unwrapped data. Either 'overlap', where product overlap is minimized, or '2stage', where minimization is done on connected components, are allowed methods. Default is 'overlap'.")
    ...
    parser.add_argument('-xRes', '--xRes', dest='xRes', default=None, help="Pixel size in x.")
    parser.add_argument('-yRes', '--yRes', dest='xRes', default=None, help="Pixel size in y.")
...
def prep_dem(demfilename, bbox_file, prods_TOTbbox, proj, arrshape=None, xRes=None, yRes=None, workdir='./', outputFormat='ENVI', num_threads='2'):
    '''
    ...
    if demfilename.lower()=='download':
        ...
        if xRes is None:
            ...
            gdal.Warp(demfilename, os.path.join(workdir,'SRTM_3arcsec_uncropped.dem.vrt'), options=gdal.WarpOptions(format=outputFormat, cutlineDSName=prods_TOTbbox, outputBounds=bounds, outputType=gdal.GDT_Int16, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
    ...
    try:
        ...
        if not os.path.exists(os.path.join(workdir,os.path.basename(demfilename).split('.')[0]+'.dem')):
            ...
            if xRes is None:
                ...
                gdal.Warp(demfilename, os.path.join(workdir,os.path.basename(demfilename).split('.')[0]+'_uncropped.dem.vrt'), options=gdal.WarpOptions(format=outputFormat, cutlineDSName=prods_TOTbbox, outputBounds=bounds, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
            update_file=gdal.Open(demfilename,gdal.GA_Update)
            update_file.SetProjection(proj) ; del update_file
...
def prep_mask(product_dict, maskfilename, bbox_file, prods_TOTbbox, proj, amp_thresh=None, arrshape=None, xRes=None, yRes=None, workdir='./', outputFormat='ENVI', num_threads='2'):
    '''
    ...
    if maskfilename.lower()=='download':
        ...
        if xRes is None:
            ...
            gdal.Warp(maskfilename, os.path.join(workdir,'watermask_uncropped.msk.vrt'), options=gdal.WarpOptions(format=outputFormat, outputBounds=bounds, outputType=gdal.GDT_Byte, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
    ...
    try:
        ...
        if not os.path.exists(os.path.join(workdir,os.path.basename(maskfilename).split('.')[0]+'.msk')):
            ...
            if xRes is None:
                ...
                gdal.Warp(maskfilename, os.path.join(workdir,os.path.basename(maskfilename).split('.')[0]+'_uncropped.msk.vrt'), options=gdal.WarpOptions(format=outputFormat, cutlineDSName=prods_TOTbbox, outputBounds=bounds, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
            mask_file = gdal.Open(maskfilename,gdal.GA_Update)
        ...
        if xRes is None:
            ...
            mask=gdal.Warp('', maskfilename, options=gdal.WarpOptions(format="MEM", cutlineDSName=prods_TOTbbox, outputBounds=bounds, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
...
def export_products(full_product_dict, bbox_file, prods_TOTbbox, layers, dem=None, lat=None, lon=None, mask=None, xRes=None, yRes=None, outDir='./',outputFormat='VRT', stitchMethodType='overlap', verbose=None, num_threads='2'):
    """
    ...
    for key in layers:
        ...
        for i,j in enumerate(product_dict[0]):
            ...
            if any(":/science/grids/imagingGeometry" in s for s in j):
                ...
                finalize_metadata(outname, open_shapefile(bbox_file, 0, 0).bounds, prods_TOTbbox, dem, lat, lon, xres, yres, mask, outputFormat, verbose=verbose)
            ...
            # Extract/crop full res layers, except for "unw" and "conn_comp" which requires advanced stitching
                ...
                if outputFormat=='VRT' and mask is None:
                    ...
                    if xRes is None:
                        ...
                        gdal.Warp(outname+'.vrt', outname+"_uncropped"+'.vrt', options=gdal.WarpOptions(format=outputFormat, cutlineDSName=prods_TOTbbox, outputBounds=bounds, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
                else:
                    # building the VRT
                    gdal.BuildVRT(outname +'.vrt', j)
                    ...
                    if xRes is None:
                        ...
                        gdal.Warp(outname, outname+'.vrt', options=gdal.WarpOptions(format=outputFormat, cutlineDSName=prods_TOTbbox, outputBounds=bounds, xRes=xRes, yRes=yRes, multithread=True, options=['NUM_THREADS=%s'%(num_threads)]))
...
def finalize_metadata(outname, bbox_bounds, prods_TOTbbox, dem, lat, lon, xRes=None, yRes=None, mask=None, outputFormat='ENVI', verbose=None, num_threads='2'):
    '''
    ...
    if xRes is None:
        ...
        out_interpolated=gdal.Warp('', outname, options=gdal.WarpOptions(format="MEM", cutlineDSName=prods_TOTbbox, outputBounds=bbox_bounds, xRes=xRes, yRes=yRes, dstNodata=data_array.GetRasterBand(1).GetNoDataValue(), multithread=True, options=['NUM_THREADS=%s'%(num_threads)])).ReadAsArray()
...
def main(inps=None):
    ...
    if inps.demfile is not None:
        ...
        inps.demfile, demfile, Latitude, Longitude = prep_dem(inps.demfile, standardproduct_info.bbox_file, prods_TOTbbox, proj, arrshape=arrshape, xRes=xRes, yRes=yRes, workdir=inps.workdir, outputFormat=inps.outputFormat, num_threads=inps.num_threads)
    else:
        demfile=None ; Latitude=None ; Longitude=None

    # Extract user expected layers
    export_products(standardproduct_info.products[1], standardproduct_info.bbox_file, prods_TOTbbox, inps.layers, dem=demfile, lat=Latitude, lon=Longitude, mask=inps.mask, xRes=xRes, yRes=yRes, outDir=inps.workdir, outputFormat=inps.outputFormat, stitchMethodType='overlap', verbose=inps.verbose, num_threads=inps.num_threads)
