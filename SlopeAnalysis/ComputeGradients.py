...
def plotGradientVectors(px,py,slope,T):
	"""
	...
	east=np.arange(T.xstart,T.xend,nSkip*T.xstep)
	north=np.arange(T.ystart,T.yend,mSkip*T.ystep)

	easts,norths=np.meshgrid(east,north)

	# Resample to give 20x20 points
	vx=px[::mSkip,::nSkip]
	vy=py[::mSkip,::nSkip]

	# Plot
	Fig,ax=mapPlot(slope,cmap='viridis',pctmin=1,pctmax=99,background='auto',
			extent=T.extent,showExtent=True,cbar_orientation='horizontal',title='Vector plot')
	ax.plot(easts,norths,'r.')
	ax.quiver(easts,norths,vx,vy,
